...
graph TD
    ...
    subgraph "API Layer (API Gateway)"
        C["REST API (/books, /reading-lists, /recommendations)"]
    end

    subgraph "Compute Layer (AWS Lambda)"
        D["get-books Lambda"]
        E["reading-lists Lambda"]
        F["recommendations Lambda (AI)"]
    end

    subgraph "Data & AI Services"
        G[("DynamoDB Tables")]
        H["Amazon Bedrock (Claude 3 Haiku)"]
    end

    A -->|1. Auth Request| B
    A -->|2. Authenticated API Call| C
    C -->|3. Trigger| D
    C -->|3. Trigger| E
    C -->|3. Trigger| F
    D -->|4. Query| G
...
**Project Status**: ✅ Frontend Complete | ✅ AWS Backend Fully Implemented
